...
class ShortcodesInit {
	...
	public static function init_short_code() {
		...
		foreach ( $shortcodes as $shortcode => $function ) {
			// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound -- $shortcode values are all plugin-controlled keys starting with "rtrs-" (e.g., rtrs-review-form_shortcode_tag).
	 ...
	 * @return false|string
	 */
	public static function shortcode_wrapper( $function, $atts = [], $wrapper = [] ) {
		$default_wrapper = [
			'class'  => 'rtrs',
			'before' => null,
			'after'  => null,
		];
		$wrapper         = wp_parse_args( $wrapper, $default_wrapper );
		ob_start();
	...
	public static function review_list( $atts ) {
		...
		$total_rating           = ReviewFns::getTotalRatings( get_the_ID() );
		$total_forbidden_review = ReviewFns::getTotalGDPRForbiddenReview( get_the_ID() );
		ob_start();
		if ( $total_rating ) {
			?>
		<div class="rtrs-sorting-bar">
			<h3 class="rtrs-sorting-title">
				...
				echo wp_kses_post( apply_filters( 'rtrs_review_sorting_title', $review_title, $total_rating, get_the_ID() ) );
				?>
			</h3>
	...
	<div class="rtrs-review-box">
		...
		<?php if ( $total_forbidden_review > 0 ) { ?>
		<h3 class="rtrs-gdpr-consent-title">
			<?php
			$gdpr_consent_title = esc_html(
				sprintf(
					/* translators: %d: number of users who opted out of public review display */
					_n( '%d more user is not interested in showing their review publicly.', '%d more users are not interested in showing their reviews publicly.', $total_forbidden_review, 'review-schema' ),
					$total_forbidden_review
				)
			);
			echo wp_kses_post( apply_filters( 'rtrs_gdpr_consent_review_title', $gdpr_consent_title, $total_forbidden_review, get_the_ID() ) );
			?>
		</h3>
		<?php } ?>
		<?php

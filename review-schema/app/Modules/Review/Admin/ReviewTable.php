...
class ReviewTable extends \WP_List_Table {
	...
	/**
	 * Get comment counts for specific post types (WP-admin accurate).
	 *
	 * @return object
	 */
	public function get_comment_count() {
		global $wpdb;
		$post_types   = (array) Functions::allReviewType();
		$comment_type = 'review';
		if ( empty( $post_types ) ) {
			return (object) [
				'approved'       => 0,
		...
		// $placeholders is generated from $post_types count as a %s-sequence; all values are passed via $wpdb->prepare().
		$results      = $wpdb->get_results(
			...
			",
				array_merge( $post_types, [ $comment_type ] )
			),
			ARRAY_A
		);
		...
		foreach ( $results as $row ) {
			...
			switch ( (string) $row['comment_approved'] ) {
				...
				case 'trash':
					...
					break;
				case 'post-trashed':
					$counts['post-trashed'] += $total;
					break;
			}
			$counts['total_comments'] += $total;
		}
	...
	protected function get_views() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only comment status filter; sanitized via sanitize_text_field( wp_unslash() ).
		$comment_status = isset( $_GET['comment_status'] ) ? sanitize_text_field( wp_unslash( $_GET['comment_status'] ) ) : 'all';
	...
	function column_cb( $item ) {
		return sprintf(
			'<input type="checkbox" name="delete_reviews[]" value="%s" />',
			esc_attr( $item['id'] )
		);
	}
	...
	function column_review( $item ) {
		...
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- "review_row_actions" mirrors WP core's "comment_row_actions"; intentional WP-style hook name for compatibility.
		$actions = apply_filters( 'review_row_actions', array_filter( $actions ), $comment );

		return wp_kses_post( $item['review'] ) . $this->row_actions( $actions );
	}
	...
	public function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'post_type':
				return esc_html( ucfirst( get_post_type( $item['post_id'] ) ) );
				break;

			case 'rating':
				if ( $item[ $column_name ] ) {
					return ReviewFns::review_stars( $item[ $column_name ], true ) . '(' . esc_html( $item[ $column_name ] ) . ')';
				} else {
					return '';
				}
				break;
            case 'gdpr_consent':
                if ( 'show' === $item[ $column_name ] ){
                    return esc_html__( 'Allow GDPR Consent', 'review-schema' );
                }elseif ( 'hide' === $item[ $column_name ]){
                    return esc_html__( 'Forbid GDPR Consent', 'review-schema' );
                }else{
                    return '';
                }
                break;
			...
			case 'date':
				...
				break;
			case 'response_to':
				$product_post = get_post( $item['post_id'] );
				if ( ! $product_post || ! post_type_exists( $product_post->post_type ) ) {
					return '';
				}
				ob_start();
				?>
				...
				break;
			default:
				return esc_html( print_r( $item, true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r -- Debug fallback only.
		}
	}
	...
	public function review_data( $current_page = null ) {
		...
		$per_page     = $this->get_items_per_page( 'reviews_per_page', 5 );
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- Read-only search query; sanitized via sanitize_text_field().
		$current_page = $this->get_pagenum();
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only search query; sanitized via sanitize_text_field( wp_unslash() ).
		$search = isset( $_REQUEST['s'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['s'] ) ) : '';
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only orderby query; sanitized via sanitize_text_field( wp_unslash() ).
		$orderby = isset( $_REQUEST['orderby'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['orderby'] ) ) : '';
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only order direction; sanitized via sanitize_text_field( wp_unslash() ).
		...
		foreach ( $comments as $comment ) {
			...
			$data = [
				...
				'gdpr_consent'     => get_comment_meta( $comment->comment_ID, 'rtrs_review_gdpr_consent', true ),
	...
	public function process_bulk_action() {
		...
		if ( $doaction ) {
			if ( ! isset( $_REQUEST['delete_reviews'] ) ) {
				return;
			}
			...
			if ( ! in_array( $doaction, [ 'approve', 'unapprove', 'spam', 'unspam', 'trash', 'delete' ], true ) ) {
				...
				// phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores, WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WP core dynamic bulk-action hook; intentionally invoked to integrate with core admin pipeline.
